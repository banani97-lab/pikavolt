...
/** Public (no-login) Payment Element for an invoice balance. */
export function InvoicePayForm(props: InvoicePayFormProps) {
  return (
    <Elements
...
function InnerForm({ token, amountCents }: InvoicePayFormProps) {
  ...
  const pay = async () => {
    ...
    const { error: confirmError, paymentIntent } = await stripe.confirmPayment({
      elements,
      confirmParams: {
        return_url: `${window.location.origin}/invoice/${token}`,
      },
